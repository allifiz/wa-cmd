...
const block = [
  ...
  "function uiBareJid(jid: string): string { return jid.replace('@s.whatsapp.net', '').replace('@g.us', '').replace('@lid', ''); }",
  "function uiShortJid(jid: string): string { const id = uiBareJid(jid); return id.length > 20 ? `${id.slice(0, 10)}…${id.slice(-6)}` : id; }",
  "function uiDimJid(jid: string): string { return chalk.gray(`${uiKind(jid)}${uiAlias(jid)} · ${uiBareJid(jid)}`); }",
